...
public class BeamPathNode<T extends BlockEntity & BeamPathNode.Holder> {
	...
	public ActionResult activate(ServerPlayerEntity player) {
		...
		if (selectedNode == null) {
			...
			player.networkHandler.sendPacket(new OverlayMessageS2CPacket(Text.translatable("block.superposition.prism.binding")));
		} else if (selectedNode == this) {
			pl.setSelectedBeamNode(null);
			player.networkHandler.sendPacket(new OverlayMessageS2CPacket(Text.translatable("block.superposition.prism.recursion")));
		} else {
			pl.setSelectedBeamNode(null);
			...
			if (entity.isInput() && selectedNode.entity.isOutput()) {
				...
				player.networkHandler.sendPacket(new OverlayMessageS2CPacket(Text.translatable("block.superposition.prism.bound")));
			} else if (entity.isOutput() && selectedNode.entity.isInput()) {
				bindTo(selectedNode);
				player.networkHandler.sendPacket(new OverlayMessageS2CPacket(Text.translatable("block.superposition.prism.bound")));
			} else {
				player.networkHandler.sendPacket(new OverlayMessageS2CPacket(Text.translatable("block.superposition.prism.incompatible")));
			}
