...
import link.infra.superposition.duck.Matrix4fExt;
import link.infra.superposition.mixin.GameRendererAccessor;
...
public class PrismBlockEntityRenderer implements BlockEntityRenderer<PrismBlockEntity> {
	...
	private void renderBeam(BlockPos beamVec, Vec3d cameraVec, float hoverOffset, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light) {
		...
		Sprite sprite = MinecraftClient.getInstance().getSpriteAtlas(PlayerScreenHandler.BLOCK_ATLAS_TEXTURE).apply(new Identifier("superposition:block/beam"));
	...
	private void renderLight(Vec3d cameraVec, float hoverOffset, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light) {
		...
		Sprite spriteEnd = MinecraftClient.getInstance().getSpriteAtlas(PlayerScreenHandler.BLOCK_ATLAS_TEXTURE).apply(new Identifier("superposition:block/beam_end"));
		var vc = vertexConsumers.getBuffer(RenderLayer.getTranslucentMovingBlock());
		var mat = matrices.peek().getPosition();
